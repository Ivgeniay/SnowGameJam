...
namespace Sisus.Init
{
	...
	public abstract class WrapperInitializerBase<TWrapper, TWrapped, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument> : MonoBehaviour
		, IInitializer<TWrapped, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument>, IValueProvider<TWrapped>
		#if UNITY_EDITOR
		...
		where TWrapper : Wrapper<TWrapped>
	{
		...
		public TWrapped InitTarget()
		{
			...
			if(target is IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument> initializable && GetOrCreateUnitializedWrappedObject() is var wrappedObject)
			{
				...
				if(nullArgumentGuard.IsEnabled(NullArgumentGuard.RuntimeException))
				{
					...
					if(fifthArgument == Null) throw GetMissingInitArgumentsException(GetType(), typeof(TWrapper), typeof(TFifthArgument));
					if(sixthArgument == Null) throw GetMissingInitArgumentsException(GetType(), typeof(TWrapper), typeof(TSixthArgument));
				}
				#endif
